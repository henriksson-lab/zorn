...
- [`PrepareSharding()`](https://henriksson-lab.github.io/zorn/reference/PrepareSharding.md)
  ...
  \############################################### \#' Take debarcoded
  reads and split them into suitable numbers of shards. \#' \#' The
  reads from one cell is guaranteed to only be present in a single
  shard. \#' This makes parallel processing simple as each shard can be
  processed on \#' a separate computer. Using more shards means that
  more computers can be used. \#' \#' If you perform all the
  calculations on a single computer, having more \#' than one shard will
  not result in a speedup. This option is only relevant \#' when using a
  cluster of compute nodes. \#' \#' \#' \#' TODO if we have multiple
  input samples, is there a way to group them? \#' otherwise we will be
  reading more input files than needed. that said, \#' if we got an
  index, so if list of cells specified, it is possible to quickly figure
  out \#' out if a file is needed at all for a merge \#' \#' TODO
  Figuring out if a file is needed can be done at "planning" (Zorn)
  stage \#' \#' TODO seems faster to have a single merger that writes
  multiple output files if \#' cell list is not provided. if the
  overhead is accepted then read all input files and \#' discard cells
  on the fly \#' \#' @param inputName Name of input file: Debarcoded
  reads \#' @param outputName Name of the output file: Properly sharded
  debarcoded reads \#' \#' @return A job to be executed, or being
  executed, depending on runner settings \#' @export BascetShardifyOld
  \<- function( bascetRoot, inputName="debarcoded", includeCells=NULL,
  \############# TODO: get rid of this parameter; only support direct
  merging numOutputShards=1, outputName="filtered", overwrite=FALSE,
  ...
  stop("Found no input files") \#Include all cells if nothing else
  provided if(is.null(includeCells)) includeCells \<-
  BascetCellNames(bascetRoot, inputName, bascetInstance=bascetInstance)
  includeCells \<- unique(includeCells\$cell) \#when shardifying, we
  expect cells to appear more than once – could warn for other commands!
  print(paste("Including all the",length(includeCells), "cells"))
  \#Figure out which cell goes into which shard list_cell_for_shard \<-
  shellscriptSplitArrayIntoListRandomly(includeCells, numOutputShards)
  \#Figure out input and output file names inputFiles \<-
  file.path(bascetRoot, input_shards) outputFiles \<-
  makeOutputShardNames(bascetRoot, outputName, "tirp.gz",
  numOutputShards) if(bascetCheckOverwriteOutput(outputFiles,
  overwrite)) \#Produce the script and run the job RunJob( runner =
  runner, jobname = "Z_shardify", bascetInstance = bascetInstance, cmd =
  c( \#shellscript_set_tempdir(bascetInstance),
  ...
  else new_no_job() Prepare to shard reads by collecting statistics
  about each barcode, and filtering out cells with few reads
...
- [`SpeciesCorrMatrix()`](https://henriksson-lab.github.io/zorn/reference/SpeciesCorrMatrix.md)
...
- [`assembleBascetCommand()`](https://henriksson-lab.github.io/zorn/reference/assembleBascetCommand.md)
  : Prepare Bascet command given arguments

- [`bascetCheckOverwriteOutput()`](https://henriksson-lab.github.io/zorn/reference/bascetCheckOverwriteOutput.md)
  : Detect metadata for raw input FASTQ files

- [`detectShardsForFile()`](https://henriksson-lab.github.io/zorn/reference/detectShardsForFile.md)
  : Helper function: Figure out which shards belong together given root
  input name and extension i.e. root/name.##.ext

- [`extractstreamerExit()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerExit.md)
  : extract streamer: end an instance. the object should no longer be
  used after calling this function

- [`extractstreamerExtractTo()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerExtractTo.md)
  : extract streamer: extract to external file

- [`extractstreamerListCellsAnyFile()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerListCellsAnyFile.md)
  : extract streamer: list all cells in a given file

- [`extractstreamerLs()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerLs.md)
  : extract streamer: list all files in current Bascet zip-file

- [`extractstreamerOpen()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerOpen.md)
  : extract streamer: set which file is open

- [`extractstreamerReadNLines()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerReadNLines.md)
  : extract streamer: helper function to read N lines

- [`extractstreamerReadOneLine()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerReadOneLine.md)
  : extract streamer: helper function to read one line

- [`extractstreamerShowtext()`](https://henriksson-lab.github.io/zorn/reference/extractstreamerShowtext.md)
  : extract streamer: get content of file, assumed to be text (or this
  function crashes)
...
- [`getFastqR2fromR1()`](https://henriksson-lab.github.io/zorn/reference/getFastqR2fromR1.md)
  : Get corresponding R2 file. Assumes that the input file is R1

- [`is.bascet.file()`](https://henriksson-lab.github.io/zorn/reference/is.bascet.file.md)
  : Check that parameter is a valid bascet file

- [`is.bascet.instance()`](https://henriksson-lab.github.io/zorn/reference/is.bascet.instance.md)
  : Check that parameter is a valid bascet instance

- [`is.existing.fasta()`](https://henriksson-lab.github.io/zorn/reference/is.existing.fasta.md)
  : Check that parameter is a valid shard name

- [`is.integer.like()`](https://henriksson-lab.github.io/zorn/reference/is.integer.like.md)
...
- [`is.streamer()`](https://henriksson-lab.github.io/zorn/reference/is.streamer.md)
  : Check that parameter is a valid streamer

- [`is.valid.env.variable()`](https://henriksson-lab.github.io/zorn/reference/is.valid.env.variable.md)
...
- [`isFastq()`](https://henriksson-lab.github.io/zorn/reference/isFastq.md)
  : Check if a file is a FASTQ file

- [`isPairedFastq()`](https://henriksson-lab.github.io/zorn/reference/isPairedFastq.md)
  : Check if a file is a paired FASTQ file. Panics if the file is not a
  FASTQ at all

- [`lseq()`](https://henriksson-lab.github.io/zorn/reference/lseq.md) :
  logarithmic spaced sequence; taken from emdbook library

- [`makeOutputShardNames()`](https://henriksson-lab.github.io/zorn/reference/makeOutputShardNames.md)
  : Helper function: Generate suitable output filenames according to
  shard system i.e. root/name.##.ext
...
- [`safeDownloadMD5()`](https://henriksson-lab.github.io/zorn/reference/safeDownloadMD5.md)
  : Download a file, check MD5 to ensure success. This assumes a
  file.md5 is stored on the server

- [`shellscriptCancelJobIfFileExists()`](https://henriksson-lab.github.io/zorn/reference/shellscriptCancelJobIfFileExists.md)
  : Create a piece of script to exit a job early if file exists

- [`shellscriptMakeBashArray()`](https://henriksson-lab.github.io/zorn/reference/shellscriptMakeBashArray.md)
...
- [`shellscriptMakeOneFileExpander()`](https://henriksson-lab.github.io/zorn/reference/shellscriptMakeOneFileExpander.md)
  : Helper function that takes content of a list and generates a BASH
  script that stores the content in a temporary file during execution

- [`shellscriptSplitArrayIntoListRandomly()`](https://henriksson-lab.github.io/zorn/reference/shellscriptSplitArrayIntoListRandomly.md)
  : Helper function to take an array of elements and split it randomly
  into a number of subset lists
