...
to the number of CPUs and memory per node. The way it is done here, the default
settings are used but then overridden:
...
The major difference here is that this function will now return immediately (asynchronous execution),
...
todo -- runner can be made to block by default
