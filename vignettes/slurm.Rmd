...
set up to give memory in proportion to the number of CPUs, but this can be
unreliable if different nodes have different amount of RAM per core.

Bascet is written to avoid using large amounts of RAM when possible.
However, certain commands load databases which can be large (KRAKEN, STAR),
and the amount of memory varies greatly with database size. KRAKEN has smaller
databases that are easier to fit in memory. STAR genome references tend to be
large and beg for 32GB ram or more.
