...
The Bascet QUAST command computes standard per-cell assembly QC metrics from
contigs, including contig counts, total length, GC content, N50/L50, NG50/LG50,
and auN. It writes these metrics to the `obs` table of a Bascet HDF5 file.
...
* Note - we currently only provide a *partial translation of this tool* for the metrics we provide. A full implementation will follow in the future
...
If you already have a Seurat object, append the metrics as metadata. Cells
missing from the HDF5 files receive `NA`; existing metadata columns with the
...
* If you use this tool, please cite: Seemann T. *Abricate*. GitHub https://github.com/tseemann/abricate (please also cite the underlying database you used)
* Abricate must already be installed on the system where the job runs and available on `PATH`.
...
* Bakta must already be installed on the system where the job runs and available on `PATH`.
...
* Ariba must already be installed on the system where the job runs and available on `PATH`.
