...
### TODO recompute rnaseq3 too
  ...
  bascetRoot <- file.path("/husky/henriksson/atrandi/rnaseq5/")  
...
for(curlib in c("1","2","3")){
  ...
  saveRDS(adata, file.path(bascetRoot,paste0("adata_",curlib,".RDS")))
}
...
adata1 <- readRDS("/husky/henriksson/atrandi/rnaseq5/adata_1.RDS")
adata2 <- readRDS("/husky/henriksson/atrandi/rnaseq5/adata_2.RDS")
adata3 <- readRDS("/husky/henriksson/atrandi/rnaseq5/adata_3.RDS")
...
######## Names of top genes
sort(rowSums(adata@assays$RNA@counts), decreasing = TRUE)[1:10]
