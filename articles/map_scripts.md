...
Bascet currently hosts two types of MAP functions:
...
a per-cell tool on it, and produce an output shard. Here is the built-in
SPAdes map script as a representative example:
...
BascetMapCell(
  bascetRoot,
  withfunction = "_spades",
  inputName = "filtered",
  outputName = "spades"
)
```

This writes `spades.<shard>.zip` files containing the per-cell SPAdes
output.

## Available MAP wrappers

Each wrapper is documented in detail in the vignette listed in the last
column. That vignette covers any required databases, the matching result
loading step, and downstream usage.
...
results into R. Older script-based wrappers usually have a matching
`BascetAggregate*()` function. Newer HDF5-producing wrappers can be
loaded from their `obs` table with
[`BascetReadObsFromH5()`](https://henriksson-lab.github.io/zorn/reference/BascetReadObsFromH5.md)
and appended to a Seurat object with
[`BascetAddMetaData()`](https://henriksson-lab.github.io/zorn/reference/BascetAddMetaData.md);
see the per-tool vignette for the exact call.
...
It is easy to add new functions! The easiest way is to copy and modify
the code for an existing shell-script wrapper. You can start from a
script such as:

- SKESA, which takes FASTQ as input
