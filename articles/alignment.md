...
*Note that alignment can require a large amount of RAM.* Furthermore,
the amount of RAM typically scales with the number of threads you use.
Please see manuals for each respectively aligner to get a better idea
about how much RAM you need.

## Alignment

You can now proceed with alignment using BWAMEM2. By default, this
command outputs both unsorted and sorted BAM-files.
...
features as variables, and per-cell unmapped read counts are available
in `cnt@obs$unclassified_reads`. To load these, use the following
command that loads the files and concatenates them into a single matrix.
It can then be loaded into Seurat:
